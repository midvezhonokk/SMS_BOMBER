...
import db
import config as cfg   # <-- наш новый модуль

# ===== Конфиг из .env =====
TOKEN = cfg.TOKEN
ADMIN_CHAT_ID = cfg.ADMIN_CHAT_ID
ADMIN_CHAT_ID1 = cfg.ADMIN_CHAT_ID1
VOXDOX_CHAT_ID = cfg.VOXDOX_CHAT_ID
group_id = cfg.GROUP_ID

THREADS_LIMIT = cfg.THREADS_LIMIT
THREADS_LIMITS = cfg.THREADS_LIMIT  # оставлено для совместимости со старым кодом

chat_ids_file = cfg.CHAT_IDS_FILE
chats_ids_file = cfg.CHATS_IDS_FILE
...
def save_chat_id(chat_id):
    ...
    with open(chat_ids_file, "a+") as ids_file:
        ...
        if chat_id not in ids_list:
            ...
            log.info(f"Новый пользователь: {chat_id}")
        else:
            log.debug(f"Пользователь {chat_id} уже в базе")
...
def send_for_number1(phone): 
    ...
    try:
        ...
        log.debug("otaxi.com.ua: пропущено")
...
def spamvip2(id1, nomer):
        sms1vip11=0
        log.info(f"Запуск spamvip2: chat_id={id1}, phone={nomer}")
        tell = nomer
        tel = nomer[1:]
        sms = 0
        sms1vip11 = 0
...
def spamvip1(id1, nomer):
            ...
            while sms1vip11 <= 200:
                ...
                sms1vip11 += 1
                time.sleep(1)
                log.debug(f"Итерация: {sms1vip11}")
            bot.status[str(id1)] = 0
            handle = open("kolihestvo.txt", "r")
            data = handle.read()
            sms1 = int(data) + int(sms)
...
def start_spam1(chat_id, phone_number, force):
    ...
    with open(num_file, "a+") as ids_file:
        ...
        log.info(f"Номер {nomer} добавлен в очередь спама")
...
— БОЛЬШЕЕ КОЛИЧЕСТВО ВРЕМЕНИ ДЛЯ СПАМА 🔗''',
            ...
            try:
                pay_url = payment.create_payment_link(message.chat.id)
            except Exception as e:
                bot.send_message(message.chat.id, "😔 Не удалось создать платёж, попробуйте позже.")
                print("YooKassa error:", e)
                return

            pay_keyboard = types.InlineKeyboardMarkup()
            pay_button = types.InlineKeyboardButton(text="💳 Перейти к оплате", url=pay_url)
            ...
            bot.send_message(
                message.chat.id,
                f'💰 <b>Стоимость VIP-доступа: {payment.PAYMENT_PRICE} ₽</b>\n\n'
                f'Нажмите кнопку ниже, чтобы перейти к оплате.\n'
                f'После успешной оплаты доступ будет выдан автоматически в течение минуты.',
